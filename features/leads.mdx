...
| **Processing** | Actively moving through the distribution pipeline. |
...
| **Withheld** | On hold because you've hit your [subscription threshold](/features/billing#what-happens-at-100). Will process automatically once capacity frees up. |
| **Marketplace** | Available for customers to claim rather than being pushed to a specific buyer. |
| **Audit** | Flagged for manual review based on your audit conditions, or submitted as test data. |
...
If your distribution plan includes a [Real Estate API](/features/integrations#real-estate-api) enrichment stage, successfully enriched leads display an "Enriched Data" section below the property details. This section shows:
...
When you delete a lead, any associated distribution flows are preserved for historical reference. The distribution flow's timeline will show when the lead was deleted, and the Lead column will display "Deleted" instead of the lead's name.
...
When importing, you'll select a [lead category](/features/lead-categories). The import form automatically shows all fields from that category—both standard fields (name, email, phone, address) and any custom fields you've configured. Map your CSV columns to the appropriate fields.

Validation rules are pulled from your [lead category configuration](/features/lead-categories). Fields marked as required in the lead category will be required during import. This keeps your imported leads consistent with leads that come in through the API.

### Enriching leads

If you have a [Real Estate API integration](/features/integrations#real-estate-api) configured, you can enrich leads with additional property and owner data. The **Enrich with Real Estate API** button appears in the lead edit page header.
...
Leads submitted through a source with [test mode](/features/sources#test-mode) enabled are marked as test data. These leads:
...
[Deterministic test zips](/features/sources#testing-with-zip-codes) produce specific statuses depending on the zip code used. Leads submitted with any other zip code are forced to **Audit** status to prevent distribution.

Test leads let vendors verify their integration works correctly without affecting production data or customer billing.
