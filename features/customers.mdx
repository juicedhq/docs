...
- **Sort by wallet balance** to find your best-funded customers or the accounts that need attention first
- **Filter by a minimum or maximum wallet balance** to target a funding range
...
| **Inactive** | Wallet funding is exhausted (no bid can cover its minimum) |
...
You can test webhooks before going live using either:

- **Existing lead data** — Select a real lead from the customer's history to see exactly what your endpoint will receive
- **Sample data** — Generate fake data based on a lead type to test without using real information

This lets you verify your endpoint handles the payload correctly before enabling the webhook.
...
<CardGroup cols={2}>
  <Card title="Onboarding customers" icon="user-plus" href="/guides/onboarding-customers">
    The three ways customers land in your tenant — self-serve, invite, and admin-created.
  </Card>
  <Card title="Managing customer finances" icon="wallet" href="/guides/managing-customer-finances">
    Fund a wallet, remove funds, and set up auto-recharge.
  </Card>
</CardGroup>
