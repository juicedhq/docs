...
The operators available depend on the field type you select. Here are some common operators:

| Operator | Available for | Description |
|----------|---------------|-------------|
| Equals | All field types | Exact match |
| Not equals | All field types | Does not match |
| Contains | Text fields | Value contains the specified text |
| Not contains | Text fields | Value does not contain the specified text |
| Less than | Numeric, date fields | Value is less than (or before) |
| Greater than | Numeric, date fields | Value is greater than (or after) |
